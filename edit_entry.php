...
if (isset($period))
  $end_period = $period;
if (!isset($edit_type))
  $edit_type = ''; // édition individuelle ou d'une série
$flag_periodicite = FALSE; // indique s'il faut afficher les informations de la série associée (si TRUE) vs le formulaire de périodicité
$page = verif_page();
// horaires
...
// echo "<br>heure début : ".$start_hour." : ".$start_min;
// die();
//$rep_num_weeks = '';
//$rep_month_abs1 = 0;
//$rep_month_abs2 = 1;
...
if (isset($id)) // édition d'une réservation existante (modification ou copie)
{
  ...
  if ($entry_type >= 1) // entrée associée à une périodicité
  {
    ...
    {
      $rep_start_day   = date('d', $row['start_time']);
      $rep_start_month = date('m', $row['start_time']);
      $rep_start_year  = date('Y', $row['start_time']);
      $rep_start_hour  = date('H', $row['start_time']);
      $rep_start_min   = date('i', $row['start_time']);
      $duration    = $row['end_time']-$row['start_time'];
    ...
    {
      $flag_periodicite = TRUE; // on affichera les informations et pas le formulaire
...
else // nouvelle réservation
{
  ...
  {
    $now = mktime($start_hour, $start_min, 0, $month, $day, $year);
...
if ($enable_periods == 'y')
{
  ...
  echo '<select id="period" name="period" class="form-control">'.PHP_EOL;
...
if ($type_affichage_reser == 0) // sélection de la durée
{
  ...
  echo '<b>  <input name="all_day" type="checkbox" value="yes" />'.get_vocab("all_day");
  if ($enable_periods != 'y')
...
else // sélection de l'heure ou du créneau de fin
{
  ...
  if ($enable_periods=='y')
  {
    echo "<label for='end_period'>".get_vocab("period")."</label>".PHP_EOL;
    echo "<select name='end_period' id='end_period' class='form-control'>";
    foreach ($periods_name as $p_num => $p_val)
    {
      echo "<option value=\"".$p_num."\"";
...
{
  if (($edit_type == "series") || (isset($flag_periodicite) && !$flag_periodicite)) // affiche le formulaire
  {
    ...
    if (Settings::get("show_holidays") == 'Oui'){ // on n'affiche ce choix que si les jours fériés et les vacances sont définis
      ...
      echo '<div>'.PHP_EOL;
      echo '<p><b>'.get_vocab("Holidays").'</b></p>';
  ...
  elseif(isset($flag_periodicite) && $flag_periodicite) // affiche les informations de périodicité
  {
