...
// global $area, $id_site;
// echo "paramètres ".$_GET['id_site']." ".$_GET['area'];
if (isset($_GET['id_site'])){
    if (isset($_GET['area'])){
        if ($_GET['area'] != "all"){
            $area = mysqli_real_escape_string($GLOBALS['db_c'], $_GET['area']);
			settype($area, "integer");
			$id_site = mrbsGetAreaSite($area); // corrige une incohérence au cas où les paramètres seraient forcés
        }
        else {
...
if ($_GET['pview'] != 1)
{
	...
	echo "<td>\n";
	echo make_site_select_html('year-dev.php',$id_site,$from_year,$from_month,$day,getUserName());
	echo make_area_select_html('year-dev.php',$id_site, $area, $from_year, $from_month, $day, getUserName());
	echo "</td>\n";
	echo "<td><form method=\"get\" action=\"year-dev.php\">";
	echo "<table border=\"0\">\n";
...
// construit la liste des domaines
