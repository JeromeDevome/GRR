...
function accumulate(&$row, &$count, &$hours, $report_start, $report_end, &$room_hash, &$breve_description_hash, $csv = "n")
{
  ...
  //Ajoute le nombre d'heures quand la ressource est utilisée.
...
if (($summarize != 4) && ($summarize != 5) && ($summarize != 6))
{
  ...
  if ($_GET['pview'] != 1)
  {
    ...
    echo '<input type="radio" name="summarize" value="6"';
    if ($summarize == 6) echo " checked=\"checked\"";
    echo " />" . get_vocab("dlcsv2");
    echo '<br />';
    echo get_vocab("dlcsv2_explain");
    echo '</td></tr>';
  ...
  echo '</form>';
  echo '</div>';
// Fin de if (($summarize != 4) and ($summarize != 5) and ($summarize != 6))
}
// Résultats:
if (isset($_GET["is_posted"]))
{
  ...
  $sql = "SELECT distinct e.id, e.start_time, e.end_time, e.name, e.description, "
    ...
    . ", a.area_name, r.room_name, r.description, a.id as area_id, e.overload_desc, r.order_display, t.type_name"
  . ", e.beneficiaire_ext"
  ...
  if (($nmatch == 0) && ($summarize != 4) && ($summarize != 5) && ($summarize != 6))
  {
  ...
  else
  {
    ...
    if($summarize == 6)// fichier csv adapté à un réimport
    {
      //Télécharger le fichier CSV
      header('Content-Encoding: UTF-8');
      header("Content-Type: application/csv-tab-delimited-table; charset=UTF-8'");
      ...
      {
        // Ligne d'en-tête
        echo html_entity_decode($vocab["date"]).";".html_entity_decode($vocab["start_time"]).";".html_entity_decode($vocab["end_time"]).";".html_entity_decode($vocab["room"]).";".html_entity_decode($vocab["namebooker"]).";".html_entity_decode($vocab["type"]).";";
        echo "\n";
        // traitement des données 
        for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
        {
          // date du jour
          echo date("Y-m-d",$row[1]).";";
          // heure de début
          echo date("H\hi",$row[1]).";";
          // heure de Fin
          echo date("H\hi",$row[2]).";";
          //Ressource
          echo (removeMailUnicode($row[9])) . ";";
          //Brève description
          echo (removeMailUnicode(affichage_lien_resa_planning($row[3], $row[0]))) . ";";
          // Type
          echo (removeMailUnicode($row[5])) . ";";
          echo "\r\n";
        }
      }
      grr_sql_free($res);
