...
if ((!isset($_GET['pview'])) or ($_GET['pview'] != 1))
{
	...
	<div class='ligne23'>
		<div class=\"left\">
			<button class=\"btn btn-default btn-xs\" onclick=\"javascript: location.href='month_all2.php?year=$yy&amp;month=$ym&amp;area=$area';\" ><span class=\"fas fa-backward\"></span> ".get_vocab("monthbefore")." </button>
		</div>";
		include "./include/trailer.inc.php";
		echo "<div class=\"right\">
			<button class=\"btn btn-default btn-xs\" onclick=\"javascript: location.href='month_all2.php?year=$ty&amp;month=$tm&amp;area=$area';\" >".get_vocab('monthafter')." <span class=\"fas fa-forward\"></span></button>
		</div>
	</div>";
}
...
if ($_GET['pview'] != 1)
    echo " <a href=\"month_all.php?year=$year&amp;month=$month&amp;area=$area\" title=\" ".get_vocab('default_room_month_all')." \"><span class='fas fa-sync'></span></a>";
echo "</div>";
echo "</caption>";
                ...
                if ($display_day[$cweek] == 1)
                {
                    ...
                    else
                    {
                        ...
                        if ((($auth_user_level > 1) || (auth_visiteur($user_name, $row["id"]) == 1)) 
                            ...
                            && $_GET['pview'] != 1){
                            if ($enable_periods == 'y')
                                echo '<a href="edit_entry.php?room=',$row["id"],'&amp;period=&amp;year=',$year,'&amp;month=',$month,'&amp;day=',$cday,'&amp;page=month_all2" title="',get_vocab("cliquez_pour_effectuer_une_reservation"),'"><span class="fas fa-plus"></span></a>',PHP_EOL;
                            else
                                echo '<a href="edit_entry.php?room=',$row["id"],'&amp;hour=',$hour,'&amp;minute=0&amp;year=',$year,'&amp;month=',$month,'&amp;day=',$cday,'&amp;page=month_all2" title="',get_vocab("cliquez_pour_effectuer_une_reservation"),'"><span class="fas fa-plus"></span></a>',PHP_EOL;;
                        }
                    }
                    echo "</td>\n";
                }
