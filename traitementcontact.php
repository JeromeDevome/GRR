...
if (empty($_POST['captcha']))
    $message .= "Le code captcha \\n";
//$message .="OK?";
if ($message !='')
    $message = $msg_erreur.$message;
else {// les paramètres ont été entrés,
    $input = array_map('clean_input',$_POST);
    // le code captcha est-il correct ?
    session_start();
    if($input['captcha'] != $_SESSION['captcha'])
        $message = "Le code captcha saisi est incorrect";
    else{
        // le créneau demandé est-il libre ?
