...
else if (!$etape)
{
  ...
  {// avant de continuer, on vérifie les droits sur les dossiers
    $dossiers = array('images','export','temp','themes/perso/css');
    $ok = true;
    foreach($dossiers as $dest){
      $ok1 = false;
      ...
      {
        $ok = false;
        $msg .= "Répertoire \"".$dest."\" : ERREUR de droits.<br>";
      }
    }
    if($ok)
      header("Location: ./install_mysql.php?etape=1");
    else{
      echo "Une erreur de droits en écriture a été détectée. Veuillez lire le message ci-dessous et corriger les droits.";
      echo "<br/>".$msg;
      echo "<br/>"."Après avoir corrigé les droits, vous pourrez reprendre l'installation.";
    }
  }
}
?>
