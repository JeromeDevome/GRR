...
$use_tls = (isset($_POST["use_tls"]))? ($_POST["use_tls"] == 'y') : FALSE;
...
if($etape == 0){// page d'accueil
  if (isset($_POST['reg_ldap_statut']))
  {
    if ($_POST['ldap_statut'] == "no_ldap")
    {
      $req = grr_sql_command("delete from ".TABLE_PREFIX."_setting where NAME = 'ldap_statut'");
      $grrSettings['ldap_statut'] = '';
    }
    else
    {
      if (!Settings::set("ldap_statut", $_POST['ldap_statut']))
        $erreur.= encode_message_utf8($vocab['save_err']." ldap_statut !<br />");
      $grrSettings['ldap_statut'] = $_POST['ldap_statut'];
    }
    if (isset($_POST['Valider1']))// données du formulaire complet
    {
      ...
      if (!Settings::set("ldap_champ_nom", $_POST['ldap_champ_nom']))
        $erreur.= $vocab['save_err']." ldap_champ_nom !<br />";
      ...
      if (!Settings::set("ldap_champ_prenom", $_POST['ldap_champ_prenom']))
        $erreur.= $vocab['save_err']." ldap_champ_prenom !<br />";
      ...
      if (!Settings::set("ldap_champ_email", $_POST['ldap_champ_email']))
        $erreur.= $vocab['save_err']." ldap_champ_email !<br />";
      ...
      if (!Settings::set("se3_liste_groupes_autorises", $_POST['se3_liste_groupes_autorises']))
        $erreur.= $vocab['save_err']." se3_liste_groupes_autorises !<br />";
      $grrSettings['se3_liste_groupes_autorises'] = $_POST['se3_liste_groupes_autorises'];
    }
  }
  // test des paramètres enregistrés
  if (@file_exists("../include/config_ldap.inc.php"))
  {
    $test_connexion_annuaire = '';
    $test_recherche_annuaire = '';
    ...
    if (($ldap_adresse != '') && ($ldap_port != ''))
    {
      ...
      if ($ds == "error_1")
        $test_connexion_annuaire = 'LDAPV3impossible';
      else if ($ds == "error_2")
        $test_connexion_annuaire = 'TLSimpossible';
      else if ($ds == "error_3")
        $test_connexion_annuaire = 'connOK_idKO';
      else if ($ds == "error_4")
        $test_connexion_annuaire = 'connKO';
      else if (!$ds)
        $test_connexion_annuaire = 'Echec';
      else
      {
        $test_connexion_annuaire = 'OK';
        $result = grr_ldap_search_user($ds, $ldap_base, "objectClass", "*",$ldap_filter,"y");
        if ($result == "error_1")
        {
            if ($ldap_filter == "")
                $test_recherche_annuaire = 'testConnChemAnnErr1';
            else
                $test_recherche_annuaire = 'testConnChemAnnErr1bis';
        }
        else if ($result == "error_2")
        {
            if ($ldap_filter == "")
                $test_recherche_annuaire = 'testConnChemAnnErr2';
            else
                $test_recherche_annuaire = 'testConnChemAnnErr2bis';
        }
        else
            $test_recherche_annuaire = 'OK';
        }
    }
  }
}
elseif ($etape == 1){// formulaire de configuration
...
elseif ($etape == 3)
{
    ...
    {
        if ($ldap_filter == "")
            $erreur.= "<p>".encode_message_utf8(get_vocab('cheminNonValide'))."</p><br />";
        else
            $erreur.= "<p>".encode_message_utf8(get_vocab('cheminOuFiltreNonValide'))."</p><br />";
    }
    else if ($result == "error_2")
    {
        if ($ldap_filter == "")
            $erreur.= "<p>".encode_message_utf8(get_vocab('cheminOKetResultatKO'))."</p><br />";
        else
            $erreur.= "<p>".encode_message_utf8(get_vocab('cheminFiltreOKetResultatKO'))."</p><br />";
    }
...
{// formulaire de configuration
    ...
    echo "<input type=\"text\" name=\"login_ldap\" value=\"".$ldap_login."\" size=\"40\" autocomplete='off' /><br />";
    echo "<b>".get_vocab('pwd').get_vocab('deux_points')."</b><br />";
    echo encode_message_utf8(get_vocab('ldapPwdWarning'))."<br />";
    echo "<input type=\"password\" name=\"pwd_ldap\" value=\"".$ldap_pwd."\" size=\"40\" autocomplete='off' /><br /></div>\n";
...
elseif ($etape == 0)
{
  if (!(function_exists("ldap_connect")))
  {
    ...
    {// formulaire complet si ldap_statut défini
    ...
    {// formulaire réduit si ldap_statut non défini <==> LDAP inactif
  ...
  if (@file_exists("../include/config_ldap.inc.php"))
  {
    ...
    if ($test_connexion_annuaire != 'OK'){
      echo "<li>";
      returnmsg('danger',get_vocab('testConnAnnuaire').get_vocab('deux_points'), get_vocab('Echec'), get_vocab($test_connexion_annuaire));
      echo "<div class=\"alert alert-danger\" role=\"alert\">".encode_message_utf8(get_vocab('ldapPath').get_vocab('deux_points'))."<b> ".$ldap_base."</b></div></li>";
    }
    elseif($test_connexion_annuaire == 'OK'){
      echo "<li>";
      returnmsg('success',get_vocab('testConnAnnuaire').get_vocab('deux_points'), 'OK', '');
      echo encode_message_utf8(get_vocab('ldapPath').get_vocab('deux_points'))."<b> ".$ldap_base."</b></li>";
      if ($ldap_filter!="")
        $ldap_filter_text = $ldap_filter;
      else
        $ldap_filter_text = "non";
      if(($test_recherche_annuaire != 'OK')&&($ldap_filter!="")){
        echo "<li>";
        returnmsg('danger',get_vocab('testRechChemAnn').get_vocab('deux_points'), get_vocab('Echec'), get_vocab($test_recherche_annuaire));
        echo "<div class=\"alert alert-danger\" role=\"alert\">".encode_message_utf8(get_vocab('filtreLdapSupp').get_vocab('deux_points'))."<b> ".$ldap_filter_text."</b></div></li>";
      }
      elseif($test_recherche_annuaire == 'OK'){
  ...
  elseif(Settings::get("ldap_statut") != '')
