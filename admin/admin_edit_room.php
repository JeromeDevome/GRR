 ...
 * Dernière modification : $Date: 2024-03-17 16:55$
...
if (isset($room))
{
  ...
  if ((authGetUserLevel($user_id,$area_id,"area") >= 4) || (authGetUserLevel($user_id,$room) >= 4))
    echo "<input class=\"form-control\" type=\"text\" name=\"room_name\" maxlength=\"60\" size=\"40\" value=\"".clean_input($row["room_name"])."\" />\n";
  else {
  ...
  echo "</td></tr>\n";
  // Description
  echo "<tr><td>".get_vocab("description")."</td><td><input class=\"form-control\" type=\"text\" name=\"description\"  maxlength=\"60\" size=\"40\" value=\"".clean_input($row["description"])."\" /></td></tr>\n";
  // Domaine
  ...
  {    // les creneaux sont bases sur le temps : on ne peut pas changer une ressource de domaine
    ...
    if ($nb_area > 1)
    {
      ...
      foreach($res as $row1)
      {
        ...
        echo '>'.htmlspecialchars($row1['area_name'], ENT_QUOTES);
        echo '</option>'."\n";
      }
      grr_sql_free($res);
      echo "</select></td></tr>";
...
if ((!empty($id_area)) || (isset($add_area)))
{
  ...
  // Affectation à un site : si aucun site n'a ete affecte
    ...
    else
    {
      ...
      if (clean_input($_POST['enable_periods']) == 'y')
      {
  ...
  echo "<div>";
    ...
    if (Settings::get("module_multisite") == "Oui")
    {
      ...
      if ($nb_site > 1)
      {
        ...
        foreach($res as $row1)
        {
          ...
          if ($id_site == $row1['id'])
            echo ' selected="selected"';
          echo '>'.htmlspecialchars($row1['sitename'], ENT_QUOTES);
          echo '</option>'."\n";
        }
        grr_sql_free($res);
        echo "</select></td></tr>";
      ...
      {
        $site_name = grr_sql_query1("SELECT sitename FROM ".TABLE_PREFIX."_site WHERE id=? ","i",[$id_site]);
    ...
    $sql_periode = grr_sql_query("SELECT num_periode, nom_periode FROM ".TABLE_PREFIX."_area_periodes where id_area=? order by num_periode","i",[$id_area]);
