...
$titre = (isset($_GET['titre']))? SecuChaine::CleanInput($_GET['titre']) : "";
...
if(($titre != "")&&(!preg_match("/^[a-zA-Z]/",$titre))){
  ...
  fatal_error(1,$msg); // mériterait un traitement d'erreur plus propre
	...
	if ($selection != -1)
	{
		...
		{
			grr_sql_query("delete from ".TABLE_PREFIX."_calendrier_jours_cycle WHERE DAY = ".$newdate."");
			grr_sql_query("insert into ".TABLE_PREFIX."_calendrier_jours_cycle set Jours ='".SecuChaine::ProtectDataSql($titre)."', DAY = ".$newdate."");
		}
	}
