 ...
 * Dernière modification : $Date: 2024-08-27 18:18$
...
function entre_reservation($room_id,$jour_semaine,$name,$description,
    ...
    // $rep_month_abs1 et $rep_month_abs2 sont utilisés pour les jours cycles, je les initialise au hasard
    $rep_month_abs1 = 0;
    $rep_month_abs2 = 0;
			...
			)
    {
      ...
      mrbsCreateRepeatingEntrys($starttime, $endtime,   $rep_type, $rep_enddate, $rep_opt, $room_id, $create_by, $beneficiaire, $beneficiaire_ext, $name, $type, $description, $rep_num_weeks, $option_reservation,$overload_data, $entry_moderate,$rep_jour_c, $courrier, $rep_month_abs1, $rep_month_abs2);
		...
		if ($error_duree_max_resa_area == 'yes') {
			...
			$duree_max_resa_area = grr_sql_query1("select duree_max_resa_area from ".TABLE_PREFIX."_area where id=?","i",[$area_id]);
			print_header();
...
if(!isset($_POST['step'])) //rien n'est défini
  $etape = 0;
elseif($_POST['step']==1) //le fichier est-il chargé et correct ?
{
  $etape = 1;
  // on commence par vérifier que le fichier est bien chargé et de type adéquat
  ...
  if(is_uploaded_file($xml_file['tmp_name'])&&(count($split) == 2)&&(strtolower(end($split)) =='xml')) {
    $msg = "Chargement réussi !";

    $source_file=$xml_file['tmp_name'];
    $dest_file="../images/edt.xml";
    $res_copy=copy("$source_file" , "$dest_file"); 
    if(!$res_copy){
      $err1 = "La copie du fichier a échoué : vérifiez que le dossier /images est accessible en écriture !";
    }
    else{
      $msg1 = "La copie du fichier vers le dossier temporaire a réussi.";
    }
  }
  else { 
    $err = "Le chargement du fichier a échoué.";
  }
}
elseif($_POST['step']==2) // premier test sur le fichier xml récupéré
{
  ...
  if(!$edt_xml) {
    $err = "ECHEC du chargement du fichier avec simpleXML.";
  }
  else{ // le fichier a pu être chargé, on va l'analyser
    $msg = "Début de l'analyse du fichier";
    $nom_racine=$edt_xml->getName();
    $test1 = mb_strtoupper($nom_racine)!='TABLE';
    if($test1){
        $err1 = "ERREUR: Le fichier XML fourni n'a pas l'air d'être un fichier XML EDT. Sa racine devrait être 'TABLE'.";
    }
  ...
  if(!$edt_xml) {
    ...
    foreach ($edt_xml->children() as $cours) { // les entités de premier niveau s'appellent 'cours'
      foreach($cours->attributes() as $key => $value) {
        ...
        $jour_semaine = $joursemaine[substr(mb_strtolower($tab_cours[$i]['enfant']['jour']),0,3)]; 
        ...
        if(!entre_reservation($room_id,$jour_semaine,$name,$description,$data['beg_day'],$data['beg_month'],$data['beg_year'],$hour,$minute,$data['beg_day'],$data['beg_month'],$data['beg_year'],$end_hour,$end_minute,$data['end_day'],$data['end_month'],$data['end_year'],$rep_semaine))
          $out_html.= "Erreur dans la réservation numéro ".$i.": ".$erreur.'<br />' ;
        else 
          $out_html.= "Réservation effectuée</br>";
            
      }
    }
    // on va nettoyer le fichier temporaire
    fclose($fp);
    unlink($dest_file);
  }
}
...
elseif($etape==1){
  if(isset($err))
    echo "<p class='avertissement'>".$err."</p>\n";
  else{
    echo "<p>".$msg."</p>";
    if(!$res_copy)
      echo "<p class='avertissement'>".$err1."</p>\n";
    else{
      echo "<p>".$msg1."</p>\n";
      echo '<br />';
      echo "<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' method='post'>\n";
...
elseif($etape==2) { // premier test sur le fichier xml récupéré
  echo "étape ".$etape;
  if(!$edt_xml) {
      echo "<p class='avertissement'>".$err."</p>\n";
  }
    ...
    echo "étape ".$etape.' : réservation <br />';
    if(!$edt_xml) {
      echo "<p class='avertissement'>".$err."</p>\n";
    }
    else {
