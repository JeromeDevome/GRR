 ...
 * Dernière modification : $Date: 2022-10-14 14:26$
...
if (isset($_POST['maj']) || isset($_GET['force_maj']) || $majscript)
{
	...
	if ($version_old < "1.9.5.1") {
		...
		if ($req == -1)
			$result_inter .= traiteRequete("INSERT INTO ".TABLE_PREFIX."_setting VALUES ('jour_debut_Jours/Cycles', '1');");
		$req = grr_sql_query1("SELECT VALUE FROM ".TABLE_PREFIX."_setting WHERE NAME='nombre_jours_Jours/Cycles'");
		if ($req == -1)
			$result_inter .= traiteRequete("INSERT INTO ".TABLE_PREFIX."_setting VALUES ('nombre_jours_Jours/Cycles', '1');");
    ...
    {
        $result .= formatResult("Mise à jour jusqu'à la version 3.4.3:","<b>","</b>");

        $result_inter .= traiteRequete("ALTER TABLE `".TABLE_PREFIX."_log` CHANGE `REMOTE_ADDR` `REMOTE_ADDR` VARCHAR(40) NOT NULL DEFAULT ''");
        $result_inter .= traiteRequete("ALTER TABLE `".TABLE_PREFIX."_entry` ADD INDEX( `room_id`)");
    ...
    $result_inter .= traiteRequete("INSERT INTO ".TABLE_PREFIX."_setting VALUES ('versionRC', '".$version_grr_RC."');");
